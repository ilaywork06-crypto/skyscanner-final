...
import { EMPTY_PLACEHOLDER, UiChip, UiValueViewer as ValueViewerDialog } from '../../index'
...
/**
 * The palette that colours each value by itself rather than painting the whole list one colour.
 *
 * A column of a vocabulary - the industries a row belongs to, the schemas it is declared by - is only
 * readable at a glance if each member keeps its own colour, which is what the single chip has always done.
 * Every other list stays one colour, because its values are not a vocabulary and colouring them would be
 * decoration rather than information.
 */
const PER_VALUE_PALETTE = 'industry'
...
/**
 * Pick the colour of one chip of the list, which is its own only where the column asked for that.
 */
const tokenFor = (value: string): string =>
  props.params.palette === PER_VALUE_PALETTE
    ? taxonomyToken(value, readContext(props.params).taxonomy)
    : token.value
