  ...
  <div class="sky-page">
    ...
    <div class="sky-page__content">
      ...
      >
        ...
        and clearing this browser's data removes them. At most {{ MAX_SNAPSHOTS }} are kept.
      </v-alert>
...
const { industries, assumptions, fields, findIndustry } = useRegister()
...
const rows = computed<GridRow[]>(() => assumptions.value.map((row) => assumptionToRow(row)))
...
const driftOf = (snapshot: SnapshotSummary): string => {
  const difference = rows.value.length - snapshot.rowCount
...
const driftToken = (snapshot: SnapshotSummary): string =>
  rows.value.length === snapshot.rowCount ? 'status-positive' : 'status-pending'
